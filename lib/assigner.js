...
module.exports = class Assigner {
  ...
  async init () {
    ...
    this.log.info('added these setting', this.settings)
  }
  ...
  async assign (){
    ...
    if(this.settings['add-owner-assignee']) {
      ...
      this.allAssignees = owner
    } else {
      this.log.info('not set up to assign owner... Skipping.')
      return false
    }

    this.log.info('we have these assignees to be added', this.allAssignees)

    this.context.github.issues.addAssignees(this.context.issue({ assignees: [this.allAssignees] }))
  }
}
